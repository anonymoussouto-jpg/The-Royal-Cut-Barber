...
import { Scissors, ShoppingBag, Calendar, User, MessageSquare, LayoutDashboard, MapPin, Clock, Crown, LogOut } from "lucide-react";
...
function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: whatsappNumber } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('system_settings').select('value').eq('key', 'whatsapp_number').maybeSingle();
      if (!data) return null;
      try {
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
             ...
             {session ? (
               <DropdownMenu>
                 <DropdownMenuTrigger asChild>
                   <Button variant="ghost" className="relative h-10 w-10 rounded-full border border-primary/20 bg-primary/10">
                 ...
                 <DropdownMenuContent className="w-56 bg-zinc-950 border-white/10 text-white" align="end">
                   ...
                   <DropdownMenuSeparator className="bg-white/5" />
                   <DropdownMenuItem className="cursor-pointer text-red-500 focus:bg-red-500 focus:text-white" onClick={handleSignOut}>
      ...
      {whatsappNumber && (
        <a
          ...
          className="fixed bottom-24 right-6 z-50 p-4 bg-green-500 text-white rounded-full shadow-2xl shadow-green-500/20 hover:bg-green-600 hover:scale-110 transition-all duration-300 md:bottom-6"
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-xl border-t border-border/40 px-4 py-2">
        <div className="flex items-center justify-between max-w-md mx-auto h-14">
          ...
          <Link to={session ? "/perfil" : "/login"} className={`flex flex-col items-center justify-center gap-1 w-full min-w-[44px] min-h-[44px] transition-colors ${isLinkActive('/perfil') || isLinkActive('/login') ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
            <User className="w-5 h-5" />
            <span className="text-[9px] font-bold uppercase">{session ? 'Perfil' : 'Entrar'}</span>
          </Link>
